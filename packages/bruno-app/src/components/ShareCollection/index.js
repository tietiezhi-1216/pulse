...
const ShareCollection = ({ onClose, collectionUid }) => {
  ...
  const handleExportZip = async () => {
    try {
      ...
      if (result.success) {
        toast.success(t('SHARE_COLLECTION.EXPORTED'));
      }
    } catch (error) {
      toast.error(t('SHARE_COLLECTION.EXPORT_FAILED', { message: error.message }));
    }
  };
  ...
  return (
    <Modal size="lg" title={t('COLLECTION_SETTINGS.OVERVIEW.SHARE_COLLECTION')} handleCancel={onClose} hideFooter>
      <StyledWrapper className="flex flex-col">
        <p className="text-sm mb-4">
          ...
          {' '}- {t('SHARE_COLLECTION.OPEN_FORMAT_DESC')}
        </p>

        {/* Bruno Format Section */}
        <div className="section-title">{t('SHARE_COLLECTION.PULSE_FORMAT')}</div>
        <div className="bruno-format-grid mb-6">
          ...
          >
            <div className="card-header">
              <span className="card-title">{t('SHARE_COLLECTION.PULSE_ZIP')}</span>
            ...
            <div className="feature-list">
              <div className="feature-item">
                ...
                <span>{t('SHARE_COLLECTION.ZIP_FEATURE_STRUCTURE')}</span>
              </div>
              <div className="feature-item">
                <IconCheck size={14} className="checkmark" />
                <span>{t('SHARE_COLLECTION.ZIP_FEATURE_COLLABORATE')}</span>
              </div>
              <div className="feature-item">
                <IconCheck size={14} className="checkmark" />
                <span>{t('SHARE_COLLECTION.ZIP_FEATURE_OPEN')}</span>
              </div>
            </div>
          ...
          >
            <div className="card-header">
              <span className="card-title">{t('SHARE_COLLECTION.SINGLE_YAML')}</span>
            </div>
            <p className="card-description">{t('SHARE_COLLECTION.YAML_DESCRIPTION')}</p>
            <div className="feature-list">
              <div className="feature-item">
                <IconCheck size={14} className="checkmark" />
                <span>{t('SHARE_COLLECTION.YAML_FEATURE_SINGLE')}</span>
              </div>
              <div className="feature-item">
                <IconCheck size={14} className="checkmark" />
                <span>{t('SHARE_COLLECTION.YAML_FEATURE_SHARE')}</span>
              </div>
            </div>
        ...
        <div className="other-format-grid">
          ...
          >
            ...
            <div className="format-info">
              ...
              <div className="format-description">{t('SHARE_COLLECTION.EXPORT_FOR_POSTMAN')}</div>
            </div>
          </div>
        </div>
        ...
        {selectedFormat === EXPORT_FORMATS.POSTMAN && hasNonExportableRequestTypes.has && (
          <div className="flex items-center mt-4 p-3 rounded" style={{ backgroundColor: 'rgba(251, 191, 36, 0.1)' }}>
            ...
            <span className="text-sm" style={{ color: '#f59e0b' }}>
              {t('SHARE_COLLECTION.NON_EXPORTABLE_NOTE', { types: hasNonExportableRequestTypes.types.join(', ') })}
            </span>
          </div>
        )}
        ...
        <div className="modal-footer">
          <Button
            ...
            loading={isExporting}
          >
            {isExporting ? t('SHARE_COLLECTION.EXPORTING') : t('COMMON.CONTINUE')}
          </Button>
        </div>
      </StyledWrapper>
    </Modal>
