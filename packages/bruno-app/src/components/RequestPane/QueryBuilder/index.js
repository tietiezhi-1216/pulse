        ...
        <div className="schema-empty-state">
          {schemaError ? (
            <>
              ...
              <div className="empty-state-title">{t('REQUEST_PANEL.QUERY_BUILDER.FAILED_TO_LOAD_SCHEMA')}</div>
              ...
              <div className="empty-state-actions">
                ...
                >
                  {t('REQUEST_PANEL.QUERY_BUILDER.UPLOAD_SCHEMA_FILE')}
                </Button>
              </div>
            </>
          ) : (
            <>
              <div className="empty-state-title">{t('REQUEST_PANEL.QUERY_BUILDER.NO_SCHEMA_LOADED')}</div>
              <div className="empty-state-description">
                {t('REQUEST_PANEL.QUERY_BUILDER.NO_SCHEMA_LOADED_DESCRIPTION')}
              </div>
              <div className="empty-state-actions">
                <Button
                  variant="outline"
                  ...
                  onClick={() => loadSchema('introspection')}
                >
                  {t('REQUEST.LOAD_FROM_INTROSPECTION')}
                </Button>
                <Button
                  variant="outline"
                  color="secondary"
        ...
        <div className="sync-error-banner">
          ...
          <div className="sync-error-text">
            {syncError === 'multiple_operations' ? (
              <>
                <strong>{t('REQUEST_PANEL.QUERY_BUILDER.MULTIPLE_OPERATIONS_TITLE')}</strong>
                <span>{t('REQUEST_PANEL.QUERY_BUILDER.MULTIPLE_OPERATIONS_DESCRIPTION')}</span>
              </>
            ) : null}
          </div>
        </div>
    ...
    <ErrorBoundary>
      <StyledWrapper>
        <div className="query-builder-search">
          <input
            type="text"
            placeholder={t('REQUEST_PANEL.QUERY_BUILDER.SEARCH_OPERATIONS')}
            value={searchText}
            className="mousetrap"
                ...
                {isExpanded && !isDisabled && (
                  fields.length > 0 ? (
                    ...
                    <div className="empty-state">
                      {searchText ? t('REQUEST_PANEL.QUERY_BUILDER.NO_MATCHING_FIELDS') : t('REQUEST_PANEL.QUERY_BUILDER.NO_FIELDS_AVAILABLE')}
                    </div>
                  )
                )}
              </div>
