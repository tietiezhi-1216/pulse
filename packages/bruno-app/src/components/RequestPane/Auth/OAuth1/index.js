...
const placementLabelKeys = {
  header: 'AUTH_FIELDS.HEADER',
  query: 'REQUEST.QUERY_PARAMS',
  body: 'AUTH_FIELDS.BODY'
};
...
const OAuth1 = ({ item = {}, collection, request, save, updateAuth }) => {
  ...
  const handlePrivateKeyChange = (val) => {
    if (val && /^@file\(/.test(val.trim())) {
      toast.error(t('OAUTH1.FILE_REFERENCE_UPLOAD_ONLY'));
      return;
    }
  ...
  return (
    <StyledWrapper className="mt-2 flex w-full gap-4 flex-col">
      ...
      <div className="flex items-center gap-4 w-full">
        <label className="block min-w-[140px]">{t('OAUTH1.CONSUMER_KEY')}</label>
      ...
      {!oauth1.signatureMethod?.startsWith('RSA-') && (
        <div className="flex items-center gap-4 w-full">
          <label className="block min-w-[140px]">{t('OAUTH1.CONSUMER_SECRET')}</label>
      ...
      <div className="flex items-center gap-4 w-full">
        <label className="block min-w-[140px]">{t('OAUTH2.TOKEN')}</label>
        <div className="single-line-editor-wrapper flex-1">
          <SingleLineEditor
      ...
      <div className="flex items-center gap-4 w-full">
        <label className="block min-w-[140px]">{t('OAUTH1.TOKEN_SECRET')}</label>
        <div className="single-line-editor-wrapper flex-1 flex items-center">
          <SingleLineEditor
      ...
      <div className="flex items-center gap-4 w-full">
        <label className="block min-w-[140px]">{t('OAUTH1.SIGNATURE_METHOD')}</label>
      ...
      {oauth1.signatureMethod?.startsWith('RSA-') && (
        <div className="flex items-start gap-4 w-full">
          <label className="block min-w-[140px] mt-1">{t('OAUTH1.PRIVATE_KEY')}</label>
          {isFileRef ? (
            ...
            <div className="flex flex-1 flex-col gap-2">
              ...
              <div className="flex flex-row gap-2">
                <button
                  ...
                  title={t('PREFERENCES.GENERAL.SELECT_FILE')}
                  type="button"
                >
                  <IconUpload size={14} />
                  <span className="text-xs">{t('OAUTH1.UPLOAD_FILE')}</span>
                </button>
              </div>
            </div>
          )}
      ...
      <div className="flex items-center gap-4 w-full">
        <label className="block min-w-[140px]">{t('OAUTH1.ADD_PARAMS_TO')}</label>
        <div className="inline-flex items-center cursor-pointer oauth1-dropdown-selector">
          <MenuDropdown
            items={Object.entries(placementLabelKeys).map(([value, labelKey]) => ({
              id: value,
              label: t(labelKey),
          ...
          >
            <div className="flex items-center justify-end oauth1-dropdown-label select-none">
              {t(placementLabelKeys[oauth1.placement] || placementLabelKeys.header)}
              <IconCaretDown className="caret ml-1 mr-1" size={14} strokeWidth={2} />
            </div>
          </MenuDropdown>
        </div>
      ...
      {oauth1.placement === 'body' && (
        <div className="flex items-center gap-4 w-full">
          ...
          <span className="text-xs opacity-60">
            {t('OAUTH1.BODY_PLACEMENT_WARNING')}
          </span>
        </div>
      )}
      ...
      {advancedOpen && (
        <>
          ...
          <div className="flex items-center gap-4 w-full">
            <label className="block min-w-[140px]">{t('OAUTH1.VERIFIER')}</label>
            <div className="single-line-editor-wrapper flex-1">
              <SingleLineEditor
          ...
          <div className="flex items-center gap-4 w-full">
            <label className="block min-w-[140px]">{t('OAUTH1.TIMESTAMP')}</label>
            <div className="single-line-editor-wrapper flex-1">
              <SingleLineEditor
          ...
          <div className="flex items-center gap-4 w-full">
            <label className="block min-w-[140px]">{t('OAUTH1.NONCE')}</label>
            <div className="single-line-editor-wrapper flex-1">
              <SingleLineEditor
                value={oauth1.nonce || ''}
                theme={storedTheme}
          ...
          <div className="flex items-center gap-4 w-full">
            <label className="block min-w-[140px]">{t('OAUTH1.VERSION')}</label>
            <div className="single-line-editor-wrapper flex-1">
              <SingleLineEditor
          ...
          <div className="flex items-center gap-4 w-full">
            <label className="block min-w-[140px]">{t('OAUTH1.REALM')}</label>
            <div className="single-line-editor-wrapper flex-1">
              <SingleLineEditor
                value={oauth1.realm || ''}
                theme={storedTheme}
