    ...
    <div className="mb-2 border dark:border-gray-700 rounded-lg overflow-hidden">
      ...
      {isExpanded && (
        <div className="p-3">
          ...
          {decodedToken && (
            <div className="mt-3">
              <div className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">{t('OAUTH2.DECODED_PAYLOAD')}</div>
...
const ExpiryTimer = ({ expiresIn }) => {
  ...
  return (
    ...
    >
      {timeLeft > 0 ? t('OAUTH2.EXPIRES_IN', { time: formatExpiryTime(timeLeft) }) : t('OAUTH2.EXPIRED')}
    </div>
  );
};
  ...
  return (
    <StyledWrapper className="relative w-auto h-fit mt-2">
      {Object.keys(creds)?.length ? (
        creds?.error ? (
          <pre className="text-red-600 dark:text-red-400">{t('OAUTH2.TOKEN_FETCH_ERROR_DETAILS')}</pre>
        ) : (
          <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-sm">
            <TokenSection title={t('OAUTH2.ACCESS_TOKEN')} token={creds.access_token} />
            <TokenSection title={t('OAUTH2.REFRESH_TOKEN')} token={creds.refresh_token} />
            <TokenSection title={t('OAUTH2.ID_TOKEN')} token={creds.id_token} />
            {(creds.token_type || creds.scope) ? (
              <div className="mt-3 p-2 bg-gray-50 dark:bg-gray-800 rounded-lg text-xs">
                <div className="grid grid-cols-2 gap-2">
                  {creds.token_type ? (
                    <div className="flex items-center space-x-1">
                      <span className="font-medium">{t('OAUTH2.TOKEN_TYPE')}:</span>
                      <span className="text-gray-600 dark:text-gray-300">{creds.token_type}</span>
                    </div>
                  ) : null}
                  {creds?.scope ? (
                    <div className="flex items-center space-x-1 min-w-0">
                      <span className="font-medium flex-shrink-0">{t('OAUTH2.SCOPE')}:</span>
        ...
        <div className="text-gray-500 dark:text-gray-400">{t('OAUTH2.NO_TOKEN_FOUND')}</div>
      )}
    </StyledWrapper>
  );
};
