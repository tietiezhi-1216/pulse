...
const Oauth2ActionButtons = ({ item, request, collection, url: accessTokenUrl, credentialsId }) => {
  ...
  const handleFetchOauth2Credentials = async () => {
    ...
    try {
      ...
      if (!result || !result.access_token) {
        const errorMessage = result?.error || t('OAUTH2.NO_ACCESS_TOKEN_RECEIVED');
        console.error(errorMessage);
        toast.error(errorMessage);
        return;
      }

      toast.success(t('OAUTH2.TOKEN_FETCHED'));
    } catch (error) {
      ...
      toast.error(error?.message || t('OAUTH2.FETCH_TOKEN_ERROR'));
    } finally {
  ...
  const handleRefreshAccessToken = async () => {
    ...
    try {
      ...
      toast.success(t('OAUTH2.TOKEN_REFRESHED'));
    } catch (error) {
      console.error(error);
      toggleRefreshingToken(false);
      toast.error(error?.message || t('OAUTH2.REFRESH_TOKEN_ERROR'));
    }
  };
  ...
  const handleClearCache = (e) => {
    dispatch(clearOauth2Cache({ collectionUid: collection?.uid, url: interpolatedAccessTokenUrl, credentialsId }))
      .then(() => {
        toast.success(t('OAUTH2.CLEAR_CACHE_SUCCESS'));
      })
      .catch((err) => {
  ...
  const handleCancelAuthorization = async () => {
    try {
      ...
      if (result.success && result.cancelled) {
        toast.error(t('OAUTH2.AUTHORIZATION_CANCELLED'));
        toggleFetchingToken(false);
        toggleFetchingAuthorizationCode(false);
      }
    } catch (err) {
      console.error('Error cancelling authorization:', err);
      toast.error(t('OAUTH2.CANCEL_AUTHORIZATION_ERROR'));
    }
  };
