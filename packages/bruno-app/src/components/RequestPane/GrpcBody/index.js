...
const MessageToolbar = ({
  ...
  return (
    <div className="message-toolbar">
      <span className="message-label">{t('REQUEST.MESSAGE_LABEL', { number: index + 1 })}</span>
      <div className="toolbar-actions mr-2">
        <ToolHint text={t('REQUEST.FORMAT_JSON')} toolhintId={`prettify-msg-${index}`}>
          <button onClick={onPrettify} className="toolbar-btn">
            <IconWand size={16} strokeWidth={1.5} />
          </button>
        </ToolHint>

        <ToolHint text={t('REQUEST.GENERATE_SAMPLE')} toolhintId={`regenerate-msg-${index}`}>
        ...
        {canClientStream && (
          <ToolHint text={isConnectionActive ? t('REQUEST.SEND_MESSAGE') : t('REQUEST.CONNECTION_NOT_ACTIVE')} toolhintId={`send-msg-${index}`}>
            <button
              onClick={onSend}
        ...
        {showDelete && (
          <ToolHint text={t('REQUEST.DELETE_MESSAGE')} toolhintId={`delete-msg-${index}`}>
...
const SingleGrpcMessage = ({ message, item, collection, index, methodType, handleRun, canClientSendMultipleMessages, isLast }) => {
  ...
  const onRegenerateMessage = async () => {
    try {
      ...
      if (!methodPath) {
        toastError(new Error(t('REQUEST.METHOD_PATH_NOT_FOUND')));
        return;
      }
      ...
      if (result.success) {
        ...
        toast.success(t('REQUEST.SAMPLE_MESSAGE_GENERATED'));
      } else {
        toastError(new Error(result.error || t('REQUEST.SAMPLE_MESSAGE_FAILED')));
      }
    } catch (error) {
    ...
    return (
      <StyledWrapper>
        <div className="empty-state">
          <p>{t('REQUEST.NO_GRPC_MESSAGES')}</p>
          <Button
