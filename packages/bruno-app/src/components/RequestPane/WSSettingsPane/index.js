...
const WSSettingsPane = ({ item, collection }) => {
  ...
  const formErrors = {
    timeout: isNaN(Number(connectionTimeout)) && t('WEBSOCKET.TIMEOUT_INVALID'),
    keepAliveInterval: isNaN(Number(keepAliveInterval)) && t('WEBSOCKET.TIMEOUT_INVALID')
  };
  ...
  return (
    <StyledWrapper className="flex flex-col gap-4 w-full">
      <section className="grid gap-4 items-center grid-cols-2">
        <div>
          <label className="font-medium mb-2">{t('WEBSOCKET.TIMEOUT')}</label>
          <InfoTip
            ...
            content={(
              <div>
                <p>
                  <span>{t('WEBSOCKET.TIMEOUT_HINT')}</span>
                </p>
              </div>
            )}
          />
        ...
        <div>
          <label className="font-medium mb-2">{t('WEBSOCKET.KEEP_ALIVE_INTERVAL')}</label>
          <InfoTip
            ...
            content={(
              <div>
                <p>
                  <span>
                    {t('WEBSOCKET.KEEP_ALIVE_HINT')}
                  </span>
                </p>
                <p className="mt-2">{t('WEBSOCKET.ZERO_OFF')}</p>
              </div>
            )}
          />
        </div>
