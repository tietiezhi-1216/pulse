  ...
  const handleKeyDown = (e) => {
    ...
    if (!tagNameRegex.test(text)) {
      setError(isBruFormat
        ? t('REQUEST_PANEL.SETTINGS.TAG_BRU_INVALID')
        : t('REQUEST_PANEL.SETTINGS.TAG_INVALID')
      );
      return;
    }
    if (tags.includes(text)) {
      setError(t('REQUEST_PANEL.SETTINGS.TAG_EXISTS', { tag: text }));
      return;
    }
    if (handleValidation) {
  ...
  return (
    <StyledWrapper className="flex flex-wrap flex-col gap-2">
      <SingleLineEditor
        ...
        value={text}
        placeholder={t('REQUEST_PANEL.SETTINGS.TAG_PLACEHOLDER')}
          ...
          ? tags.map((_tag) => (
              <li key={_tag}>
                ...
                >
                  ...
                  <span className="tag-remove" title={t('COMMON.REMOVE_TAG')} onClick={() => handleRemoveTag(_tag)}>
