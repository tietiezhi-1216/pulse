...
const GenerateCodeItem = ({ collectionUid, item, onClose, isExample = false, exampleUid = null }) => {
  ...
  const modalTitle = isExample
    ? t('RESPONSE_EXAMPLE.GENERATE_CODE_FOR', {
        name: get(item, 'draft.examples', []).find((e) => e.uid === exampleUid)?.name || t('RESPONSE_EXAMPLE.EXAMPLE')
      })
    : t('RESPONSE_EXAMPLE.GENERATE_CODE');
          ...
          <div className="editor-container">
            {validateURLWithVars(finalUrl) ? (
              ...
              <div className="error-message">
                <h1>{t('RESPONSE_EXAMPLE.INVALID_URL', { url: finalUrl })}</h1>
                <p>{t('RESPONSE_EXAMPLE.CHECK_URL')}</p>
              </div>
            )}
          </div>
        </div>
