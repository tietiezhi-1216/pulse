...
const ProxySettings = ({ collection }) => {
  ...
  const validateHostnameOnChange = (hostname) => {
    if (hostname && hostname.length > 1024) {
      toast.error(t('COLLECTION_SETTINGS.PROXY.VALIDATION.HOSTNAME_MAX'));
      return false;
    }
    return true;
  };
  ...
  const validatePortOnChange = (port) => {
    ...
    if (isNaN(portNum)) {
      toast.error(t('COLLECTION_SETTINGS.PROXY.VALIDATION.PORT_NUMBER'));
      return false;
    }
    if (portNum < 1 || portNum > 65535) {
      toast.error(t('COLLECTION_SETTINGS.PROXY.VALIDATION.PORT_RANGE'));
      return false;
    }
    return true;
  };
  ...
  const validateAuthUsernameOnChange = (username) => {
    if (username && username.length > 1024) {
      toast.error(t('COLLECTION_SETTINGS.PROXY.VALIDATION.USERNAME_MAX'));
      return false;
    }
    return true;
  };
  ...
  const validateAuthPasswordOnChange = (password) => {
    if (password && password.length > 1024) {
      toast.error(t('COLLECTION_SETTINGS.PROXY.VALIDATION.PASSWORD_MAX'));
      return false;
    }
    return true;
  };
  ...
  const validateBypassProxyOnChange = (bypassProxy) => {
    if (bypassProxy && bypassProxy.length > 1024) {
      toast.error(t('COLLECTION_SETTINGS.PROXY.VALIDATION.BYPASS_MAX'));
      return false;
    }
    return true;
  };
        ...
        <div className="mb-3 flex items-center">
          <label className="settings-label flex items-center" htmlFor="enabled">
            ...
            <InfoTip infotipId="request-var">
              <div>
                <ul>
                  <li><span style={{ width: '50px', display: 'inline-block' }}>{t('COLLECTION_SETTINGS.PROXY.INHERIT')}</span> - {t('COLLECTION_SETTINGS.PROXY.INHERIT_HELP')}</li>
                  <li><span style={{ width: '50px', display: 'inline-block' }}>{t('COLLECTION_SETTINGS.PROXY.ENABLED')}</span> - {t('COLLECTION_SETTINGS.PROXY.ENABLED_HELP')}</li>
                  <li><span style={{ width: '50px', display: 'inline-block' }}>{t('COLLECTION_SETTINGS.PROXY.DISABLED')}</span> - {t('COLLECTION_SETTINGS.PROXY.DISABLED_HELP')}</li>
                </ul>
              </div>
            </InfoTip>
          </label>
