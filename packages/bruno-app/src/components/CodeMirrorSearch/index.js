    ...
    <StyledWrapper>
      <div className="bruno-search-bar">
        <input
          ...
          placeholder={t('CODE_SEARCH.PLACEHOLDER')}
          spellCheck={false}
          onKeyDown={(e) => {
        ...
        />
        <span className="searchbar-result-count">{matchCount > 0 ? `${matchIndex + 1} / ${matchCount}` : t('CODE_SEARCH.ZERO_RESULTS')}</span>
        <ToolHint text={t('CODE_SEARCH.REGEX_SEARCH')} toolhintId="searchbar-regex-toolhint" place="top">
          <button className={`searchbar-icon-btn ${regex ? 'active' : ''}`} onClick={handleToggleRegex}><IconRegex size={16} /></button>
        </ToolHint>
        <ToolHint text={t('CODE_SEARCH.CASE_SENSITIVE')} toolhintId="searchbar-case-toolhint" place="top">
          <button className={`searchbar-icon-btn ${caseSensitive ? 'active' : ''}`} onClick={handleToggleCase}><IconLetterCase size={14} /></button>
        </ToolHint>
        <ToolHint text={t('CODE_SEARCH.WHOLE_WORD')} toolhintId="searchbar-wholeword-toolhint" place="top">
          <button className={`searchbar-icon-btn ${wholeWord ? 'active' : ''}`} onClick={handleToggleWholeWord}><IconLetterW size={14} /></button>
        </ToolHint>
        <button className="searchbar-icon-btn" title={t('COMMON.PREVIOUS')} onClick={handlePrev}><IconArrowUp size={14} /></button>
        <button className="searchbar-icon-btn" title={t('COMMON.NEXT')} onClick={handleNext}><IconArrowDown size={14} /></button>
