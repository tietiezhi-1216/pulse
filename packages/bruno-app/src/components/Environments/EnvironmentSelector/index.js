...
const TABS = [
  { id: 'collection', labelKey: 'COMMON.COLLECTION', icon: <IconDatabase size={16} strokeWidth={1.5} /> },
  { id: 'global', labelKey: 'ENVIRONMENTS.GLOBAL', icon: <IconWorld size={16} strokeWidth={1.5} /> }
];

const EMPTY_STATE_DESCRIPTIONS = {
  collection: 'ENVIRONMENTS.EMPTY_COLLECTION_DESC',
  global: 'ENVIRONMENTS.EMPTY_GLOBAL_DESC'
};
...
const DropdownTrigger = forwardRef(({ collectionEnv, globalEnv }, ref) => {
  ...
  if (!hasAnyEnv) {
    return (
      ...
      >
        <span className="env-text-inactive max-w-36 truncate no-wrap">{t('ENVIRONMENTS.NO_ENVIRONMENT')}</span>
...
const EnvironmentSelector = ({ collection }) => {
  ...
  const handleEnvironmentSelect = (environment) => {
    ...
    dispatch(action)
      .then(() => {
        toast.success(environment ? t('ENVIRONMENTS.CHANGED_TO', { name: environment.name }) : t('ENVIRONMENTS.NONE_ACTIVE'));
        hideDropdown();
      })
      .catch(() => {
