...
const ModifyCookieModal = ({ onClose, domain, cookie }) => {
  ...
  const onSubmit = async () => {
    try {
      ...
      const errMsg = error.message || t('COOKIES.PARSE_COOKIE_ERROR');
      toast.error(errMsg);
    }
  };
  ...
  useEffect(() => {
    ...
    setParsedCookie();
  }, [isRawMode, cookieString, dispatch, formik, t]);
          ...
          {isRawMode ? (
            ...
            <div className="space-y-4">
              ...
              <div className="w-full flex items-end">
                <div>
                  <label className="block mb-1">{t('COOKIES.EXPIRATION', { timezone: moment.tz.guess() })}</label>
                  <input
                    type="datetime-local"
                    name="expires"
