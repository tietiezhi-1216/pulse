...
const DeleteCookieModal = ({ onClose, cookieName, onDelete }) => {
  ...
  return (
    <Modal onClose={onClose} handleCancel={onClose} title={t('COOKIES.DELETE_COOKIE')} hideFooter={true}>
      ...
      <div className="font-normal mt-4">
        {t('COOKIES.DELETE_COOKIE_CONFIRM', { name: cookieName })}
      </div>
...
const CollectionProperties = ({ onClose }) => {
  ...
  const clearDomainCookiesAction = () => {
    dispatch(deleteCookiesForDomain(domainToClear))
      .then(() => {
        toast.success(t('COOKIES.DOMAIN_CLEAR_SUCCESS'));
      })
      .catch((err) => console.log(err) && toast.error(t('COOKIES.DOMAIN_CLEAR_ERROR')));
  ...
  const deleteCookieAction = () => {
    if (cookieToDelete) {
      ...
      dispatch(deleteCookie(domain, path, key))
        .then(() => {
          toast.success(t('COOKIES.DELETE_SUCCESS'));
        })
        ...
        <StyledWrapper>
          {!cookies || !cookies.length ? (
            ...
            <div className="flex items-center justify-center flex-col">
              ...
              <p className="empty-text mt-2">{t('COOKIES.ADD_TO_GET_STARTED')}</p>
              <Button
                type="submit"
                size="sm"
                className="mt-8"
            ...
            <div className="flex items-center justify-center flex-col">
              ...
              <p className="empty-text mt-2">{t('COOKIES.SEARCH_HINT')}</p>
            </div>
          ) : (
            // Show cookies list
                    ...
                    <Accordion.Header index={i} className="flex items-center">
                      <div className="flex items-center">
                        ...
                        <span className="domain-count ml-2 text-xs">
                          ...
                          {t(domainWithCookies.cookies.length === 1 ? 'COOKIES.COOKIE' : 'COOKIES.COOKIES')})
                        </span>
