...
const Cache = () => {
  ...
  const handleSave = useCallback(
    (newCachePreferences) => {
      ...
      ).catch(() => toast.error(t('PREFERENCES.CACHE.UPDATE_FAILED')));
    },
  ...
  const handleResetCache = () => {
    dispatch(clearHttpHttpsAgentCache())
      .then(() => toast.success(t('PREFERENCES.CACHE.CLEARED')))
      .catch(() => toast.error(t('PREFERENCES.CACHE.CLEAR_FAILED')));
  };

  return (
    <StyledWrapper className="w-full">
      <form className="bruno-form" onSubmit={formik.handleSubmit}>
        <div className="section-title mt-6 mb-3">{t('PREFERENCES.CACHE.SECTION_TITLE')}</div>
        ...
        <div className="flex items-center my-2">
          ...
          <label className="block ml-2 select-none" htmlFor="sslSession.enabled">
            {t('PREFERENCES.CACHE.ENABLE_SSL_SESSION')}
          </label>
        </div>
        <div className="text-xs mt-1 ml-6 opacity-70">
          {t('PREFERENCES.CACHE.SSL_SESSION_HELP')}
        </div>
