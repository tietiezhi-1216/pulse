...
const CreateWorkspace = ({ onClose }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      workspaceName: Yup.string()
        .trim()
        .min(1, t('WORKSPACE_FORM.VALIDATION.WORKSPACE_NAME_EMPTY'))
        .max(255, t('COLLECTION_FORM.VALIDATION.MAX_255'))
        .required(t('WORKSPACE_FORM.VALIDATION.WORKSPACE_NAME_REQUIRED'))
        .test('unique-name', t('WORKSPACE_FORM.VALIDATION.WORKSPACE_NAME_EXISTS'), function (value) {
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        toast.success(t('WORKSPACE_FORM.CREATED'));
        onClose();
      } catch (error) {
        toast.error(multiLineMsg(t('WORKSPACE_FORM.CREATE_ERROR'), formatIpcError(error)));
      } finally {
  ...
  return (
    <Modal
      size="md"
      title={t('ACTIONS.CREATE_WORKSPACE')}
      description={t('WORKSPACE_FORM.CREATE_DESCRIPTION')}
      confirmText={isSubmitting ? t('WORKSPACE_FORM.CREATING') : t('ACTIONS.CREATE_WORKSPACE')}
        ...
        <form className="bruno-form" onSubmit={formik.handleSubmit}>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <label htmlFor="workspaceFolderName" className="flex items-center font-semibold">
                  ...
                  <Help width="300">
                    <p>
                      {t('WORKSPACE_FORM.FOLDER_NAME_HELP_1')}
                    </p>
                    <p className="mt-2">
                      {t('WORKSPACE_FORM.FOLDER_NAME_HELP_2')}
                    </p>
                  </Help>
                </label>
                {isEditing ? (
          ...
          <div className="mb-4">
            <label htmlFor="workspaceLocation" className="font-semibold mb-2 flex items-center">
              ...
              <Help>
                <p>
                  {t('WORKSPACE_FORM.LOCATION_HELP_1')}
                </p>
                <p className="mt-2">
                  {t('WORKSPACE_FORM.LOCATION_HELP_2')}
                </p>
              </Help>
            </label>
            <input
