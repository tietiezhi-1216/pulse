...
const CollectionsList = ({ workspace }) => {
  ...
  const handleOpenCollectionClick = (collection, event) => {
    ...
    if (collection.isLoaded === false) {
      if (collection.isGitBacked) {
        toast.error(t('COLLECTIONS_LIST.NEEDS_CLONE', { name: collection.name }));
      } else {
        toast.error(t('COLLECTIONS_LIST.MISSING_ON_DISK', { name: collection.name }));
      }
      return;
    }
  ...
  const handleRenameCollection = (collection) => {
    ...
    if (collection.isLoaded === false) {
      toast.error(t('COLLECTIONS_LIST.CANNOT_RENAME_UNCLONED'));
      return;
    }
  ...
  const handleShareCollection = (collection) => {
    dropdownRefs.current[collection.uid]?.hide();
    if (collection.isLoaded === false) {
      toast.error(t('COLLECTIONS_LIST.CLONE_BEFORE_SHARE'));
      return;
    }
  ...
  const handleRemoveCollection = (collection) => {
    dropdownRefs.current[collection.uid]?.hide();
    if (collection.isLoaded === false) {
      toast.error(t('COLLECTIONS_LIST.CANNOT_REMOVE_UNLOADED'));
      return;
    }
    setSelectedCollectionUid(collection.uid);
  ...
  const handleDeleteCollection = (collection) => {
    dropdownRefs.current[collection.uid]?.hide();
    if (collection.isLoaded === false) {
      toast.error(t('COLLECTIONS_LIST.CANNOT_DELETE_UNLOADED'));
      return;
    }
    setSelectedCollectionUid(collection.uid);
  ...
  const handleConnectGit = (collection) => {
    dropdownRefs.current[collection.uid]?.hide();
    if (collection.isLoaded === false) {
      toast.error(t('COLLECTIONS_LIST.CANNOT_CONNECT_GIT_UNLOADED'));
      return;
    }
    setGitTarget({
  ...
  const handleCopyGitUrl = async (collection) => {
    ...
    try {
      ...
      toast.success(t('COLLECTIONS_LIST.GIT_URL_COPIED'));
    } catch (e) {
      toast.error(t('COLLECTIONS_LIST.GIT_URL_COPY_FAILED'));
    }
  };
      ...
      <div className="collections-list">
        {workspaceCollections.length === 0 ? (
          <div className="empty-state">
            ...
            <h3 className="empty-title">{t('COLLECTIONS_LIST.EMPTY_TITLE')}</h3>
            <p className="empty-description">{t('COLLECTIONS_LIST.EMPTY_DESCRIPTION')}</p>
          </div>
        ) : (
              ...
              <div className="collection-info">
                <div className="collection-header">
                  ...
                  {!isDefaultWorkspace && collection.isLoaded === false && (
                    <StatusBadge status="warning" size="xs">{t('COLLECTIONS_LIST.NOT_CLONED')}</StatusBadge>
                  )}
                </div>
                      ...
                      <>
                        {collection.isGitBacked && (
                          ...
                          >
                            ...
                            <span>{t('COLLECTIONS_LIST.COPY_GIT_URL')}</span>
                          </div>
                        )}
                        {!collection.isGitBacked && collection.isLoaded !== false && (
                          ...
                          >
                            ...
                            <span>{t('COLLECTIONS_LIST.CONNECT_TO_GIT')}</span>
                          </div>
                        )}
                        {collection.isGitBacked && (
                          <div
                          ...
                          >
                            ...
                            <span>{t('COLLECTIONS_LIST.REMOVE_GIT_REMOTE')}</span>
                          </div>
                        )}
                      </>
                    )}
